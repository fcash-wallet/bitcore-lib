...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new fcash.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new fcash.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = fcash.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new fcash.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function fcashTest() {
      block1 = fcash.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('fcash', fcashTest, {maxTime: maxTime});
